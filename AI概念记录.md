...
LLM（大型语言模型）是通过训练大量文本数据而获得的能力，能够生成人类语言。我们可以把它看成是相关AI程序�?引擎"

### 2.tokenizer
负责在用户与LLM之间进行文本与数组的转换(编码:将文本转换为token序列，解�?将token序列转换为文�?，将用户输入的文本转换为LLM可以理解的token序列，同时将LLM的输出序列转换为用户可以理解的文本�?
...
context window：上下文窗口，context能够处理的最大token�?
...
MCP（模型调用协�?**统一的接入规�?*）是指LLM与Tool之间的通信协议，用于在LLM和Tool之间传递信息和指令。MCP定义了LLM和Tool之间的交互方式，包括请求和响应的格式、错误处理机制等�?
